...
class GridLoader:
    ...
    def _direct_initialization(self, x = None, y = None, tri = None,
        ...
        assert isinstance(obc_nodes, list), 'obc_nodes must be given as a list'
        assert isinstance(cropped_nodes, np.ndarray), 'cropped nodes must be a numpy array'
        assert isinstance(cropped_cells, np.ndarray), 'cropped cells must be a numpy array'
...
class OBC:
    ...
    def x_obc(self):
        ...
        return [self.x[nodestring] for nodestring in self.nodestrings]

    @property
    def y_obc(self):
        '''y-position of nodes at the open boundary'''
        return [self.y[nodestring] for nodestring in self.nodestrings]
    
    @property
    def lat_obc(self):
        '''latitude of nodes on the open boundary'''
        return [self.lat[nodestring] for nodestring in self.nodestrings]

    @property
    def lon_obc(self):
        '''longitude of nodes on the open boundary'''
        return [self.lon[nodestring] for nodestring in self.nodestrings]
...
class ControlVolumePlotter:
    def plot_cvs(self, 
        ...
        Optional:
        ----
        - inds:      choose which control volumes to plot
        ...
        if not inds:
            inds = np.arange(0, field.shape[0])
...
class ExportGrid:
    ...
    def write_coast(self, outfolder = None, buffer_width = 50000, verbose = False):
        ...
        # Make model coastline polygons (exterior is one polygon, islands are separate polygons)
        ...
        for nodestring in self.nodestrings:
            ...
            for point_ind, buffer_ind in zip(points, ind):
                lines.append(shp.geometry.LineString([[self.x[point_ind],   self.y[point_ind]],  
                                                      [
                                                          np.array(exterior_buffer.exterior.xy[0])[buffer_ind],
                                                          np.array(exterior_buffer.exterior.xy[1])[buffer_ind],
                                                      ], 
                                                      ]))
            ...
            obc_points.append(
                shp.geometry.Point(
                    self.x[nodestring[int(len(nodestring)/2)]], 
                    self.y[nodestring[int(len(nodestring)/2)]],
                )
            ) # A point on the line
...
class PlotFVCOM:
    ...
    def georeference(
        self, 
        url='https://openwms.statkart.no/skwms1/wms.topo4.graatone?service=wms&request=getcapabilities', 
        ...
        if int(self.reference.split(':')[-1]) == 4326:
            ...
            if grid:
                self.draw_geo_grid([self.x.min(), self.x.max(), self.y.min(), self.y.max()])
    ...
    def _plot_section_line(self):
        '''
        Illustrates where a section has been drawn from
        '''
        self.plot_grid(rasterized=True)
...
Extended features through the mesh-connectivity TGE (.T) class:
    ...
    def __repr__(self):
        return f'''
{self.casename} FVCOM grid object
