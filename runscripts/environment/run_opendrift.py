...
def run(with_diffusion = False, zlevel_norkyst = False, hours_pr_release = 1, use_local = False, fallback_vdiff = 10^(-5)):
    '''
    ...
    last_file = stop + timedelta(days=1)
    file_dates = [start - timedelta(days = 1) + timedelta(days = n) for n in range((last_file + timedelta(days=1) - start).days)]
    ...
    if not zlevel_norkyst:
        if use_local:
            o = get_local_reader(file_dates, o)
        else:
            o = get_thredds_reader(file_dates, o)

    else:
        o = get_thredds_zlevel_reader(file_dates, o)
    ...
    #    o.set_config('vertical_mixing:timestep', 30.) # copied from the test cases, not sure if this is strict

    if with_diffusion: # and zlevel_norkyst:
        o.set_config('drift:vertical_mixing', True)
        o.set_config('vertical_mixing:diffusivitymodel', 'environment')
        o.set_config('vertical_mixing:timestep', 30.) # not sure if this is strict
       # o.set_config('environment:fallback:ocean_vertical_diffusivity', fallback_vdiff)
    ...
    if with_diffusion and not zlevel_norkyst:
        savename = f'drift_norkyst_unlimited_vdiff_30s_fb_{fallback_vdiff}.nc'
