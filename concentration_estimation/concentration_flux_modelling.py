...
#calculate the average weight of the particles at each timestep
#average weight of the particles at each timestep is the sum of particle_lifespan_matrix[:,:,0] along the depth dimension

#make mox loss simple, or just do the 
#def calc_loss_profile(k_ox,moles_methane,delta_t):
#    output_vector = np.zeros_like(moles_methane)
#    output_vector[0] = moles_methane[0]
#    for i in range(len(moles_methane)-1):
#        output_vector[i+1] = output_vector[i]*np.exp(-k_ox*delta_t)
#    return output_vector

loss_mox = (np.nansum(particle_lifespan_matrix[:,:,0],axis=1))*R_ox*3600
#loss_mox = calc_loss_profile(R_ox,np.nansum(particle_lifespan_matrix[:,:,0],axis=1),3600)
...
#make a small yyaxis plot of particle mass died and particle mass redistributed
#(particle_mass_redistributed and particle_mass_died)

plt.figure(figsize=(8, 6), dpi=150)
plt.plot(particle_mass_redistributed[:-10], label='Redistributed mass')
plt.plot(particles_mass_died[:-10], label='Died mass')
plt.plot(particles_mass_died[:-10]-particle_mass_redistributed[:-10], label='Difference', linestyle='--')
#plot the difference between the two on a second y-axis
plt.xlabel('Time [hours]', fontsize=14)
plt.ylabel('Mass [moles]', fontsize=14)
plt.xticks(fontsize=12)
plt.yticks(fontsize=12)
plt.xlim(0,1520)
plt.legend(fontsize=14)
plt.title('Mass redistribution and death over time', fontsize=14)
plt.grid(True, alpha=0.3)
plt.show()