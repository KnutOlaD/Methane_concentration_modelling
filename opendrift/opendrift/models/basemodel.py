...
class OpenDriftSimulation(PhysicsMethods, Timeable):
    ...
    def animation(self,
        ...
        self.__save_or_plot_animation__(plt.gcf(),
                                        ...
                                        interval=50,
                                        blit = blit)
        logger.info(f'Time to make animation: {(datetime.now() - start_time)}')
    ...
    def __save_animation__(self, fig, plot_timestep, filename, frames, fps,
        ...
        # Add a progressbar for peace of mind
        widget = [f'Writing {filename}: ', pb.Percentage(), pb.BouncingBar(), pb.ETA()]
        bar = pb.ProgressBar(widgets=widget, maxval=frames)
        bar.start()
